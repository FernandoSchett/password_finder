/*
@(#)File:           bruteforce_hibrido.c
@(#)Last changed:   Date: 11/24/2022 21:40:00
@(#)Purpose:        Brute Force Password Breaker with MultiProcessing and MultiThreading
@(#)Modified:       Fernando Schettini, Gabriel Mascarenhas, Jadson Nobre
@(#)Usage:
 (*) Hotocompile:   mpicc bruteforce_hibrido.c -o bruteforce_hibrido -fopenmp -std=c99 -O3 
 (*) Hotoexecute:   mpirun -np <number_of_processes> ./bruteforce_mpi <password> <number_of_threads_per_process>
                    mpirun -np 4 ./bruteforce_mpi senhatez 8
@(#)Comment:
...
void bruteForce(char *pass, int np, int num_processo) {
  ...
  #pragma omp parallel for schedule(static) private(j) shared(achou)
  for(j = num_processo*max/np; j != (num_processo+1)*max/np; j++){
    if(j == pass_decimal){
      int index = 0;
      while(j > 0){
